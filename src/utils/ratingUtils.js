...
export const calculateMaalemRating = (maalemOrUser, reviews = [], interventions = []) => {
  ...
  const matchedReviews = (reviews || []).filter((r) => {
    ...
    const isFallback = (mId === 'maalem-1' || mId === '22222222-2222-2222-2222-222222222222' || !r.maalem_id) && 
      (maalemOrUser.role?.toUpperCase() === 'MAALEM' || maalemOrUser.specialty);
    return matchId || matchPhone || isFallback;
  });

  // 2. Fusion avec les interventions complétées ayant une note directe si non présentes dans reviews
  ...
  const additionalFromInterventions = (interventions || [])
    .filter((i) => {
      if (!i || !i.rating || Number(i.rating) <= 0) return false;
      ...
      const isFallback = (mId === 'maalem-1' || mId === '22222222-2222-2222-2222-222222222222') && (maalemOrUser.role?.toUpperCase() === 'MAALEM');
      const isCandidate = matchIntId || matchIntPhone || isFallback;
    ...
    .map((i) => ({
      ...
      comment: i.comment || `Intervention SOS complétée avec succès (${i.rating}★).`,
      badges: i.badges || [],
