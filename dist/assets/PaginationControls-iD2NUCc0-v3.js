import{j as e,m as V}from"./motion-vendor-DzmBqWEP-v3.js";import{r as x,bq as Be,br as Ue,bd as Se,S as Pe,aF as pe,M as ze,bs as Ze,aV as Ke,ay as We,bt as qe}from"./icons-vendor-0Hd87Cxk-v3.js";import{x as Je,C as Ye,$ as J,M as Y,K as se,a as Re}from"./maplibre-vendor-v_HaOzn3-v3.js";import{u as Qe,a as Xe,x as Ge}from"./index-Br3wWzfv-v3.js";typeof window<"u"&&Re&&(Re.WORKER_URL="/assets/maplibre-gl-worker.mjs");const re={OSM_FR:{id:"OSM_FR",name:"Plan Urbain HD Maroc (Rues Nommées & Bâtiments)",tiles:["https://a.tile.openstreetmap.fr/osmfr/{z}/{x}/{y}.png","https://b.tile.openstreetmap.fr/osmfr/{z}/{x}/{y}.png","https://c.tile.openstreetmap.fr/osmfr/{z}/{x}/{y}.png"],attribution:"&copy; OpenStreetMap France & contributeurs"},OSM_HOT:{id:"OSM_HOT",name:"OSM Humanitarian Hot (Épuré Pastel)",tiles:["https://a.tile.openstreetmap.fr/hot/{z}/{x}/{y}.png","https://b.tile.openstreetmap.fr/hot/{z}/{x}/{y}.png","https://c.tile.openstreetmap.fr/hot/{z}/{x}/{y}.png"],attribution:"&copy; OpenStreetMap contributors, Humanitarian OSM"},ESRI_STREETS:{id:"ESRI_STREETS",name:"Esri World Streets HD (Officiel & Détaillé)",tiles:["https://server.arcgisonline.com/ArcGIS/rest/services/World_Street_Map/MapServer/tile/{z}/{y}/{x}"],attribution:"&copy; Esri, HERE, Garmin, OpenStreetMap"},VPS_DEDICATED:{id:"VPS_DEDICATED",name:"Serveur Dédié VPS BricoleMoi (Ultra-Rapide)",tiles:["/tiles-proxy/styles/basic-preview/{z}/{x}/{y}.png"],attribution:"&copy; BricoleMoi Dedicated VPS &copy; OpenMapTiles"},SATELLITE:{id:"SATELLITE",name:"Vue Satellite HD (Bâtiments réels)",tiles:["https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}"],attribution:"&copy; Esri, Maxar, Earthstar Geographics"}},et=[{id:"GOLD_CYAN",label:"Doré & Cyan (Haute Clarté)",iconColor:"bg-amber-400",desc:"Grands axes dorés et rues nettes"},{id:"NEON_CYBER",label:"Néon Cyberpunk (Sombre)",iconColor:"bg-cyan-400",desc:"Radar de nuit avec routes fluorescentes"},{id:"SILVER_SLATE",label:"Silver Épuré (Minimaliste)",iconColor:"bg-slate-300",desc:"Rendu monochrome moderne"},{id:"NATURAL",label:"Couleurs Naturelles (Standard)",iconColor:"bg-emerald-400",desc:"Rendu topographique officiel"}],tt=n=>{const t=re[n]||re.OSM_FR,p=typeof window<"u"&&(window.devicePixelRatio||1)>1.25;return{version:8,sources:{"base-tiles":{type:"raster",tiles:t.tiles.map(g=>g.replace("{r}",p?"@2x":"")),tileSize:256,attribution:t.attribution||"&copy; OpenStreetMap"}},layers:[{id:"base-tiles-layer",type:"raster",source:"base-tiles",minzoom:0,maxzoom:20,paint:{"raster-resampling":"linear","raster-contrast":.05,"raster-saturation":.05}}]}},E={PLUMBING:`
  ...
  `},ct=({mode:n="CLIENT_PICKER",selectedLat:t,selectedLng:p,onLocationSelect:b,filterCategory:g,activeRouteCoords:o,trackingMaalemPos:N,trackingMaalemId:L,trackingClientPos:w,etaSummary:F,distanceKm:C,durationMin:Z})=>{var Ee;const{user:v}=Qe(),{interventions:f,maalems:I,calculateDistanceInKm:B,showToast:S,isMaalemOnline:Q,toggleMaalemOnlineStatus:lt}=Xe(),ue=!!(v&&(String(v.role||"").toUpperCase()==="MAALEM"||String(v.role||"").toUpperCase()==="ADMIN")),ne=x.useRef(null),M=x.useRef(null),P=(()=>{try{const s=localStorage.getItem("bricolemoi_client_gps")||localStorage.getItem("bricolemoi_maalem_gps");return s?JSON.parse(s):null}catch{return null}})(),xe=t||(P==null?void 0:P.lat)||33.5883,fe=p||(P==null?void 0:P.lng)||-7.6328,[z,Oe]=x.useState("OSM_FR"),[he,_e]=x.useState("GOLD_CYAN"),[u,be]=x.useState({lat:xe,lng:fe}),[X,me]=x.useState(!1),[$,le]=x.useState(!1),[ge,ae]=x.useState(!1),[we,ie]=x.useState(!1),k=x.useRef({}),A=x.useRef({}),G=x.useRef(null),O=x.useRef(null),j=x.useRef(null),ve=x.useRef(null),K=x.useRef(null),U=x.useRef(null),He=x.useRef(0),R=x.useRef(null),ye=x.useRef(""),oe=x.useRef(!1);x.useEffect(()=>{if(!ne.current)return;G.current=null,O.current=null,R.current&&(cancelAnimationFrame(R.current),R.current=null),U.current=null,ve.current=null,K.current=null,j.current&&(j.current.remove(),j.current=null),k.current={},A.current={};const s=new Je({container:ne.current,style:tt(z),center:[fe,xe],zoom:14.5,minZoom:10,maxZoom:20,pitch:0,bearing:0,antialias:!0,attributionControl:!1});s.addControl(new Ye({showCompass:!0}),"top-right"),M.current=s,s.on("load",()=>{me(!0),s.resize()});const l=setTimeout(()=>{s&&s.resize()},200);return s.on("click",i=>{b&&(!o||o.length<2)&&b(i.lngLat.lat,i.lngLat.lng)}),()=>{clearTimeout(l),G.current=null,O.current=null,k.current={},A.current={},me(!1),s.remove()}},[z]),x.useEffect(()=>{o&&o.length>=2||M.current&&t&&p&&M.current.flyTo({center:[p,t],zoom:14.5,speed:1.2})},[t,p,o]),x.useEffect(()=>{const s=M.current;if(!s||!X)return;const l=!!(o&&o.length>=2);if(l)G.current&&(G.current.remove(),G.current=null);else if(G.current)G.current.setLngLat([u.lng,u.lat]),G.current.getElement().parentNode||G.current.addTo(s);else{const r=document.createElement("div");r.style.width="36px",r.style.height="36px",r.className="relative flex items-center justify-center cursor-pointer",r.innerHTML=`
        ...
        `;const d=new J({offset:25,className:"clean-trust-popup"}).setHTML(`<div class="bg-white/95 backdrop-blur-xl border border-slate-200/90 p-3 rounded-2xl text-center shadow-xl font-sans">
          ...
          </div>`);G.current=new Y({element:r}).setLngLat([u.lng,u.lat]).setPopup(d).addTo(s)}const i=parseFloat(t||(u==null?void 0:u.lat)||33.5883),c=parseFloat(p||(u==null?void 0:u.lng)||-7.6328);if(n==="CLIENT_PICKER"&&!isNaN(i)&&!isNaN(c))if(O.current)O.current.setLngLat([c,i]),O.current.setDraggable(!l),O.current.getElement().parentNode||O.current.addTo(s);else{const r=document.createElement("div");r.style.width="44px",r.style.height="52px",r.className=`${l?"":"cursor-move"} transform -translate-y-full transition-transform hover:scale-110 z-30`,r.innerHTML=`
        ...
        `;const d=new J({offset:25,className:"clean-trust-popup"}).setHTML(`<div class="bg-white/95 backdrop-blur-xl border border-slate-200/90 p-3 rounded-2xl text-center shadow-xl font-sans">
          ...
          `;const ee=new J({offset:20,className:"clean-trust-popup"}).setHTML(nt({maalem:r,isSelf:_,distanceKm:D,etaMin:H}));k.current[r.id]=new Y({element:h}).setLngLat([a,d]).setPopup(ee).addTo(s)}}),N&&Array.isArray(N)&&N.length>=2){const r=parseFloat(N[0]),d=parseFloat(N[1]);if(!isNaN(r)&&!isNaN(d)&&r>20&&r<38){const a=(I||[]).find(m=>String(m.id).trim()===String(L).trim()),_=rt({maalem:a,etaSummary:F,distanceKm:C,durationMin:Z});if(j.current){j.current.getElement().parentNode||j.current.addTo(s);const m=j.current.getPopup();m&&m.setHTML(_);const D=U.current||[d,r],[H,h]=D;if(Math.hypot(d-H,r-h)>4e-5){const te=st(h,H,r,d);He.current=te,K.current&&(K.current.style.transform=`rotate(${Math.round(te)}deg)`),R.current&&cancelAnimationFrame(R.current);const ce=performance.now(),$e=1200,Le=De=>{const Ve=De-ce,Ce=Math.min(Ve/$e,1),ke=1-Math.pow(1-Ce,3),Ae=H+(d-H)*ke,Te=h+(r-h)*ke;U.current=[Ae,Te],j.current&&j.current.setLngLat([Ae,Te]),Ce<1?R.current=requestAnimationFrame(Le):(R.current=null,U.current=[d,r])};R.current=requestAnimationFrame(Le)}else j.current.setLngLat([d,r]),U.current=[d,r]}else{const m=document.createElement("div");m.style.width="58px",m.style.height="58px",m.className="relative flex items-center justify-center cursor-pointer z-40 group",m.innerHTML=`
          ...
          `;const D=m.querySelector(".vehicle-heading-ring");K.current=D,U.current=[d,r];const H=new J({offset:30,className:"clean-trust-popup",closeButton:!1}).setHTML(_);j.current=new Y({element:m}).setLngLat([d,r]).setPopup(H).addTo(s)}}}else j.current&&(R.current&&(cancelAnimationFrame(R.current),R.current=null),j.current.remove(),j.current=null,ve.current=null,K.current=null,U.current=null);if(!ue)Object.keys(A.current).forEach(r=>{A.current[r].remove(),delete A.current[r]});else{const r=f.filter(a=>a.status==="PENDING"),d=new Set(r.map(a=>String(a.id).trim()));Object.keys(A.current).forEach(a=>{d.has(String(a).trim())||(A.current[a].remove(),delete A.current[a])}),r.forEach(a=>{const _=parseFloat(a.lat||33.5883),m=parseFloat(a.lng||-7.6328);if(isNaN(_)||isNaN(m))return;const D=Ie(a.service_type),H=B(u.lat,u.lng,_,m);if(A.current[a.id])A.current[a.id].setLngLat([m,_]),A.current[a.id].getElement().parentNode||A.current[a.id].addTo(s);else{const h=document.createElement("div");h.style.width="44px",h.style.height="44px",h.style.willChange="transform",h.className="relative flex items-center justify-center cursor-pointer transform transition-transform hover:scale-125",h.innerHTML=`
